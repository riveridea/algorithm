...
public:
	...
	//binary search 
	//find the largest index that meets
	//citations[i] >= size - i
	int start = 0;
	int size = citations.size(), end = size;
	if(end == 0) return 0;

	while(start < end)
	{
	    int mid = start + (end - start)/2;    
	    if(citations[mid] < size - mid)// this index does not meet h-index
	    {
		start = mid +1;
	    }
	    else // a valid index but need to continue to find the max
	    {
		end = mid;	
	    }
	}
