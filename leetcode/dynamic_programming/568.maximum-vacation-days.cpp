 ...
 * 3rd week : fly from city 1 to city 2 on Monday, and play 7 days.deliver.changczh_SKYLAKE_JUL25.20191003.144325
 * 
 * 
 * 
 * 
...
public:
    int maxVacationDays(vector<vector<int>>& flights, vector<vector<int>>& days) {
        //2D dp, 
        //dp[i][j] - max vacation days when spend the current week j on city i
        //start from dp[0][0]... dp[cities-1][0]
        int nc = days.size();
        if(nc == 0) return 0;
        int nw = days[0].size();
        if(nw == 0) return 0;
        ...
        if(flights[0].size() == 0) return 0;

        //start from city 0 in week 0, init the 2d dp from the first week for all 
        //possible start, as you can fly from 0 to other city in the monday
        vector<vector<int>> dp(nc, vector<int>(nw, -1));
        dp[0][0] = days[0][0];
        for(int i = 1; i < nc; i++){
        ...
        //i - week
        //j - current city
        //k - possible next city
        for(int i = 1; i < nw; i++){
            for(int j = 0; j < nc; j++){
                if(dp[j][i-1] == -1) continue; //last week, this city is not visited
                for(int k = 0; k < nc; k++)
                    //if(dp[k][i-1] != -1 && (flights[k][j] == 1 || k == j))
                    if((flights[j][k] == 1 || k == j))
                        dp[k][i] = max(dp[k][i], dp[j][i-1] + days[k][i]);
            }
        }

        //now check last week nw -1 but on different last dest
        //choose the max value
        int ans = 0;
        for(int i = 0; i < nc; i++){
            ans = max(ans, dp[i][nw-1]); 
        }
        return ans;
    }
};
