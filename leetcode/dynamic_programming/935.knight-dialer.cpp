...
//convert to 1D DP, we defineintly need two arrays, because the dependency of previous i 
//is not detemined by like i-1, i etc. It needs to be selected by nexts[i]
	vector<vector<int>> dp(2, vector<int>(10));
	fill(dp[0].begin(), dp[0].end(), 1); //j = 0 <--> N = 1

	for(int j = 1; j < N; j++){  
	    //remember to clear the current round of j array!!
	    fill(dp[j&1].begin(), dp[j&1].end(), 0);
	    for(int i = 0; i < 10; i++){
		if(nexts[i].size() == 0) continue;
		for(int n : nexts[i]){ //need last round of j
		    dp[j&1][i] += dp[~j&1][n];
		    dp[j&1][i] %= MOD;
		}
	    }
