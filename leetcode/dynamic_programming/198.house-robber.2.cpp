...
public:
	...
	for (int i = 0; i < n; i++)
	{
	    int temp = cur; //store the i-1 result to be used in next means i-2
	    cur = max(pre + nums[i], cur); //cur here is i-1 solution, pre is i-2 solution
	    pre = temp;  //updated to (i-1) solution
