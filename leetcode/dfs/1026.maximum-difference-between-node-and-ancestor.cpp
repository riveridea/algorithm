...
public:
    int maxAncestorDiff(TreeNode* root) {
        //lets first try recursive dfs        
        //the basic idea is on each path, we need to keep updateing the max and min value
        //unti the leaf and get the max - min
        //after all the paths are checked, the ans is achieved
        //the recursive way, we just need to check the left path and right path,
        //also if leaf, just return max - min
